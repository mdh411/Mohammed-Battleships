...
Game::Game(int size, int nShips) {
	...
	_enemy = new CPUHandler(_playerBoard, _enemyBoard, 1);
	...
	if (!_isSetUp) {
		...
		while (!inputDone) {
			...
			case 2: {
				...
				_player = new PlayerHandler(_playerBoard, _enemyBoard);
				_enemy = new CPUHandler(_playerBoard, _enemyBoard, _difficulty);
				std::cout << "Successfully set grid to " << _size << "x" << _size << ".\n\n";
				break;
			}
			case 3: { // set difficulty
				std::cout << "---Set Difficulty---\n";
				std::cout << "1. Easy (Default)\n";
				std::cout << "2. Normal\n";
				std::cout << "3. Hard\n";

				std::cout << "Enter a difficulty level: \n";
				int option = getValidInput(3, 0);
				if (option == -1) {
					throw new std::string("User has quit, Bye..");
				}
				_enemy = new CPUHandler(_enemyBoard, _playerBoard, option);
				_difficulty = option;
				std::cout << "Difficulty successfully set to " << std::to_string(option) << ".\n\n";
				break;
			}
